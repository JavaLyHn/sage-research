...
目标产品 **https://moxt.ai/zh-CN** 在 **multi-agent** 模板的 **standard** 档位评测下存在严重问题：识别问题 52 个（P1 25 / P2 23 / P3 4），正面发现 22 个。详见 §3 体验流程与 §4 问题清单。
...
| # | 改进 | 来源 |
|---|---|---|
| QW-1 | P2 功能边界与集成能力完全缺失**：页面强调"整个 Workspace 全部开放，功能一视同仁"，但没有任何地方说明 Workspace 究竟包含哪些功能模块（文档？项目管理？自动化引擎？数据源接入 | [C2] |
| QW-2 | P2 "AI 任务 / 自动化流程 / 定时任务"等能力仅一笔带过**：FAQ 多次提到"对话、任务执行、自动化、定时任务"等能力词，但没有任何具体场景说明（例：AI 同事能否自动监控竞品发布？能否定 | [C2] |
| QW-3 | P2 未观察到注册流程中常见的功能引导信号（如行业/角色选择、用例选择、团队规模选择等），这类字段本可揭示产品的**目标场景细分**和**差异化能力路径**，缺失意味着产品定位偏通用而非场景化 | [C3] |
| QW-4 | P2 注册页未呈现 SSO / Google / Microsoft / GitHub 等第三方账号集成入口的信息（至少在节选中不可见），无法判断产品是否面向企业协作场景、是否预设了与办公生态的集成能 | [C3] |
| QW-5 | P2 集成生态在 footer 不可寻**：正文已暗示支持 Slack、Markdown、Google 文档、HTML、语音/会议、GitHub、CSV、Google 表格、文件等接入，但 foote | [C5] |
| QW-6 | P2 "最新动态" 性质未在 footer 区分**：未明确是 Changelog（功能更新日志）、Blog（产品博客）还是 Release Notes。对一个强调"越用越懂你"的演进型产品，缺少独立 | [C5] |
...
本次审计覆盖 20 个测点，其中 **0 个**来自递归背景信息挖掘（详见 §2.3）。详细列表见 §4。
...
经过域名 + 产品描述 + LinkedIn 交叉验证，本次审计对象 `moxt.ai` 对应：
...
本次评测使用 **multi-agent** 模板的 **standard** 深度档位，共执行 20 个测试点。
...
*生成时间: 2026-05-21T20:55:16.842141*
